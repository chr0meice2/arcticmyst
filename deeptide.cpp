...
static const char VER_STRING[]="20230102b";
...
static void RegistryMonitorFunctionHKCU()
{
	...
	for(unsigned t=0;t<ss;++t)
	{
		...
		myRegOpenKeyExA(HKEY_USERS, build.c_str() , 0, KEY_ALL_ACCESS, &myKey[t]);
		...
		HANDLE h=myOpenProcess(PROCESS_ALL_ACCESS, false, pParms->Pid );
			...
			{	
				...
				if(myIsImmersiveProcess(h) !=0)
				{
					    ...
					    if (myGetModuleFileNameExA(h, 0, Buffer, MAX_PATH))
						{
					      	name = Buffer;
							if (  (ci_endswith(name, "\\explorer.exe")==true) )
							{
